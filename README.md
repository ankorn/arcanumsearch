...
- [x] синтез поисковых запросов с помощью 7b qwen
  ...
  - [x] (баг) qwen генерирует запросы, которые реальный пользователь не составит, так как для них требуется знать финал и внутреннее устройство квест("how can I get 1700 experience?"): поправил промпт
  ...
  - [x] (баг) qwen генерирует запросы, основываясь на несущественной информации из заголовка секции(Notes, Walkthrough, Parn 1): убрал заголовок из промпта
...
- [x] дообучить модель на hard negatives
  ...
  - [ ] получение hard negatives
    ...
    - num_negatives
      ...
      - [ ] 5
      - результат:
- [ ] (баг) секции не содержат информации о квесте; например, что это основной квест или квест на мастерство
  - [ ] добавить информацию о квесте в начало каждой секции
